...
def cache_args_parse(kwargs, q_len):
    ...
    dynamic = kwargs.setdefault("dynamic", 1)
...
def llama_sdpa_attn_forward_(
    ...
    **kwargs,
    ...
    cache_dense = kwargs["cache_dense"]
    dynamic = kwargs["dynamic"]
    if not init_f and not cache_dense and not dynamic:
        ...
        scores = torch.zeros_like(key_states_[..., 0])
    else:
